...
const setLoggedUser = (agentName, institutionKey, userProfile = null) => {
  ...
  const institutionLabel = getInstitutionLabel(institutionKey);
  const roleLabel = resolvedUser?.roleLabel || resolvedUser?.role || 'Não informado';
  if (profileUserName) profileUserName.textContent = agentName;
  if (profileInstitution) profileInstitution.textContent = institutionLabel || 'Instituição não informada';
...
const clearSession = () => {
  ...
  if (profileUserName) profileUserName.textContent = 'Usuário: --';
  if (profileInstitution) profileInstitution.textContent = 'Instituição: --';
